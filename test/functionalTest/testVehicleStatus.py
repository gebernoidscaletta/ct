...
def createMap(data):
    """Create a map visualization for a single data point"""
    if not data:
        return
    ...
    if not latitude or not longitude:
        log.error("No coordinates available for mapping")
        return
        
    # Create map centered on vehicle position
    m = folium.Map(location=[latitude, longitude], zoom_start=15)
    
    # Create popup content
    popup_content = f"""
        Registration: {data.get('registration')}<br>
        Speed: {data.get('speed')} km/h<br>
        Time: {data.get('timestamp')}<br>
        Ignition: {data.get('ignition')}<br>
        Location: {data.get('position_description')}<br>
        Fuel Level: {data.get('fuel_level')}%
    """
    ...
    map_file = f"D:/New/Programming/export/{timestamp}_LIVE-TRACKING_{registration}.html"
    m.save(map_file)
    log.info(f"Map visualization saved to: {map_file}")
